...
The workflow has already run `just format` and `just lint` deterministically after the developer phase and before you. Review the lint results in `.agent/lint.log`.

- **Primary source of truth:** `.agent/lint.log` (generated by the workflow) — read this file to see the exact lint output and status.
- If `.agent/lint.log` indicates lint failures (non-zero exit code), treat that as a failure and send the work back to the developer for fixes.
- Do **not** modify code during verification — your role is to report lint status, not fix it.
- If `.agent/lint.log` is missing and `just` is clearly available on PATH, you may run `just lint` as a fallback, but this is optional; the primary source is the generated log file.
